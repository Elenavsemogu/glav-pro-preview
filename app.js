...
/* ---------- Лента проезжает по кадру при попадании в экран ---------- */
...
document.querySelectorAll('[data-tabs="ind"] .tab').forEach((tab) => {
  tab.addEventListener('click', () => {
    ...
    indShot.classList.remove('sweeping');
    void indShot.offsetWidth;
    indShot.classList.add('sweeping');
    indShot.addEventListener('animationend', () => indShot.classList.remove('sweeping'), { once: true });
  });
});
